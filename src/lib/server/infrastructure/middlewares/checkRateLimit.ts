    ...
    : Promise<Response>
{
    ...
    if (!ALLOWED_METHOD.has(event.request.method)) throw redirect(303, errorRoutes.FORBIDDEN)
    ...
    if (pathname.startsWith(authRoutes.AUTH_AREA)) {
        if (pathname !== authRoutes.REDIRECT_TO_GOOGLE) {
            ...
            if (rateLimit.isFailure) throw redirect(303, rateLimit.error.description!)
        }
    }
