...
{#if options.type === "bar" && isLoading} 
  <Skeleton type='bar-chart' />
{:else if options.type === "donut" && isLoading}
  <Skeleton type='donut-chart' />
{:else}
  {#if options.type === "bar"}
    <div class="p-4 md:p-6 bg-base-100 shadow-xs rounded-lg w-full overflow-hidden min-w-0 bg-white"> 
      <div class="mb-4 flex justify-between border-b border-gray-200 pb-4 dark:border-gray-700">
        <div class="flex items-center gap-3">
          <div class="me-3 flex h-12 w-12 p-3 items-center justify-center rounded-sm bg-gray-100 dark:bg-gray-700">
              <HugeiconsIcon icon={ChartColumnBigIcon} size={28} color="#7d7d7d" strokeWidth={1.5} />
          </div>
          <div>
      ...
      <div class="grid grid-cols-2 pt-3">
        <dl class="flex items-center gap-1">
          <dt class="text-sm font-normal text-[#7d7d7d] dark:text-gray-400">Production Costs:</dt>
          <dd class="text-sm font-semibold text-gray-900 dark:text-white">$3,232</dd>
        </dl>
        <dl class="flex items-center gap-1 justify-end">
          <dt class="text-sm font-normal text-[#7d7d7d] dark:text-gray-400">Interest Rate:</dt>
  ...
  {:else if options.type === "donut"}
    <div class="p-4 md:p-6 h-full bg-base-100 card-sm shadow-xs rounded-lg bg-white">
      ...
      <div class="grid grid-cols-1 items-center justify-between border-t border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between pt-5">
          ...
          <A href="/" class="hover:text-primary-700 dark:hover:text-primary-500 rounded-lg px-0 py-1 text-sm font-semibold uppercase hover:bg-gray-100 hover:no-underline dark:border-gray-700 dark:hover:bg-gray-700 dark:focus:ring-gray-700">
            Traffic analysis
